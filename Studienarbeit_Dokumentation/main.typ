...
Im Rahmen dieser Arbeit erhält der Agent pro Zeitschritt insgesamt 31 numerische Beobachtungswerte, darunter die normalisierte Eigengeschwindigkeit, ein Bodenkontakt-Indikator (isGrounded), Zieldistanz und -richtung, vier Wand-Raycasts, ein Line-of-Sight-Flag sowie die Werte eines manuell implementierten Bodensensors. Dieser basiert auf neun abwärts gerichteten Raycasts, deren Ursprünge unter und vor dem Agenten sowie diagonal und seitlich verteilt sind, und liefert je Strahl die erkannte Bodenart (Typ-Code) und die normalisierte Distanz zum getroffenen Objekt (18 Beobachtungswerte). Die vollständige Spezifikation enthält @tab:sensorspez.
