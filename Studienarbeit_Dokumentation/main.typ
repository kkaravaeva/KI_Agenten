...
Das Gedächtnis eines LSTM wird durch einen kontinuierlich aktualisierten internen Zustand repräsentiert. Welche Informationen gespeichert, überschrieben oder vergessen werden, wird nicht manuell festgelegt, sondern während des Trainings automatisch erlernt. Im Kontext von Reinforcement Learning ermöglicht dies dem Agenten, Informationen aus vergangenen Beobachtungen zu berücksichtigen und dadurch fundiertere Entscheidungen zu treffen. In Unity ML-Agents ist LSTM bereits integriert und kann über den Konfigurationsblock memory aktiviert werden. Dadurch erhält der Agent eine Form von Gedächtnis, die es ihm erlaubt, auch in teilweise beobachtbaren Umgebungen historische Informationen in seine Entscheidungsfindung einzubeziehen.
...
Im Rahmen dieser Arbeit erhält der Agent pro Zeitschritt insgesamt 31 numerische Beobachtungswerte. Dazu zählen die normalisierte Eigengeschwindigkeit des Agenten, ein Indikator für den Bodenkontakt (isGrounded), der Richtungsvektor zum Zielobjekt sowie die Informationen eines manuell implementierten Bodensensors. Dieser basiert auf mehreren nach vorne gerichteten Raycasts und liefert sowohl Informationen über die erkannte Bodenart als auch die normalisierte Distanz zum jeweils getroffenen Objekt. //habs überarbeitet
...
Als Reward Shaping wird die Anreicherung der Belohnungsfunktion um Zwischenbelohnungen bezeichnet, etwa Strafen für Zeitverbrauch oder für das Betreten gefährlicher Felder. Ng et al. @ng_policy_1999 zeigten bereits 1999, dass potentialbasiertes Reward Shaping die optimale Policy nicht verändert, während naiv gewählte Zwischenbelohnungen unerwünschtes Verhalten hervorrufen können, beispielsweise das wiederholte Einsammeln derselben Teilbelohnung anstelle der eigentlichen Zielerreichung. @ng_policy_1999 Ein verbreitetes Grundmuster für Navigationsaufgaben ist die Kombination aus einer positiven Terminalbelohnung für das Erreichen des Ziels, negativen Terminalbelohnungen für das Scheitern sowie einer kleinen negativen Belohnung pro Zeitschritt, die als Zeitdruck wirkt und passives Verhalten verhindert. @sutton_reinforcement_2018
...
Die Trainer-Konfigurationen unterscheiden sich systematisch zwischen den Architekturen. Maßgeblich für den Vergleich sind die beiden finalen Läufe `model_comparison_final_v2.yaml` und `model_comparison_final_v3.yaml`, die jeweils alle drei Behaviors (MLP, LSTM, Transformer) in einem einzigen Lauf trainieren. Sie unterscheiden sich in zwei Punkten. Erstens weicht ausschließlich das Transformer-Behavior in vier Hyperparametern ab (beta, beta-Schedule, Learning-Rate-Schedule und Curiosity-Stärke). Zweitens unterscheidet sich die Zahl der tatsächlich ausgeführten Trainingsschritte: v2 wurde nach fünf vollständigen Curriculum-Durchläufen je Umgebung bei ca. 16,6 M Schritten gestoppt und schöpfte das in beiden Konfigurationen gesetzte Budget von 30 M Schritten nicht aus, während v3 die vollen 30 M Schritte durchlief. Die architekturbedingten Abweichungen zwischen MLP, LSTM und Transformer sowie ihre Begründung dokumentiert @sec:yaml-abweichungen. Die Bestandteile der Trainingsinfrastruktur fasst @tab:traininginfra zusammen.
...
#figure(
  table(
    ...
    [Vergleichslauf F1--F3 (maßgeblich)], [`model_comparison_final_v2.yaml` und `model_comparison_final_v3.yaml`: je drei Behaviors (MLP, LSTM, Transformer) in einem Lauf, mit identischen Basisparametern über die drei Architekturen (Abschnitt 5.3.1), die Abweichungen zwischen den Architekturen stehen ausschließlich in @tab:yaml-abweichungen. Zwischen v2 und v3 sind MLP- und LSTM-Behavior byte-identisch, die Läufe unterscheiden sich nur in:
    #enum(
      numbering: "(i)",
      [vier Transformer-Hyperparametern (beta, beta-Schedule, Learning-Rate-Schedule, Curiosity-Stärke, die Transformer-Werte in @tab:yaml-abweichungen entsprechen dem Stand v3)],
      [der erreichten Schrittzahl (v2 nach fünf Curriculum-Loops je Umgebung bei ca. 16,6 M gestoppt, das in beiden Configs gesetzte 30-M-Budget nicht ausgeschöpft, v3 volle 30 M)],
      [der effektiven num_envs (3 vs. 5, CLI-Override, YAML beidseitig 3)],
      [dem Rechner-/venv-Stand],
    )
    ],
...
Der empirische Vergleich stützt sich jedoch nicht auf einen, sondern auf zwei finale Vergleichsläufe (im Folgenden v2 und v3). Beide Läufe verwenden für MLP und LSTM identische Konfigurationen. Sie unterscheiden sich allerdings in der Transformer-Konfiguration, sowie darüber hinaus in laufweiten Randbedingungen (Trainingsbudget, Trainingsumgebung), die nicht Teil des kontrollierten Vergleichs sind. Begründung und genaue Abgrenzung erfolgen in den Abschnitten 5.2 und 5.3. Für die Auswertung folgt daraus, dass die nachstehend definierten Metriken für jeden Lauf getrennt erhoben werden und die Hypothesen H1–H3 je Lauf geprüft werden. 
...
Diese Innerhalb-Lauf-Kontrolle gilt für jeden der beiden finalen Vergleichsläufe (v2 und v3) einzeln. Keiner der beiden Läufe stellt alle drei Architekturen gleichzeitig unter optimale Bedingungen: In v3 trainiert der Transformer mit seiner best-getunten Konfiguration über das volle Trainingsbudget, während der frühere Lauf v2 den Transformer noch mit einer nicht-optimalen Konfiguration und einem kürzeren Budget fährt. MLP und LSTM sind in beiden Läufen identisch konfiguriert. 
...
Damit der Architekturvergleich aussagekräftig ist, gilt durchgehend ein Grundprinzip. Zwischen den drei Agenten darf sich nur der eigentliche Vergleichsgegenstand, also die Netzarchitektur unterscheiden, während alle anderen Bedingungen bleiben. Nur so lassen sich später beobachtete Leistungsunterschiede eindeutig auf die Architektur zurückführen und nicht auf zufällige Nebenbedingungen. Die folgenden beiden Abschnitte machen diese Trennung konkret. 5.3.1 zeigt, was für alle Architekturen identisch gehalten wird, während 5.3.2 die wenigen bewussten erforderlichen Abweichungen begründet.
...
MLP und LSTM werden in beiden finalen Läufen mit identischer Konfiguration gefahren. Der Transformer wird dagegen in zwei Konfigurationen geführt. Die in Lauf v3 verwendete Konfiguration ist die best-getunte Variante. Sie ist das Ergebnis der in Abschnitt 5.4 dokumentierten Tuning-Iterationen. Für sie gilt der Begriff „Best vs. Best". Der frühere Lauf v2 verwendet für den Transformer eine nicht-optimale Konfiguration. Für diese Architektur handelt es sich in v2 also gerade nicht um einen Best-vs-Best-Vergleich. Beide Konfigurationen werden ausgewiesen, weil beide Läufe berichtet werden. Die Konsequenzen für die Interpretation stehen in Abschnitt 5.5. Die folgende Tabelle listet sämtliche Abweichungen. Die vier Parameter, in denen sich die Transformer-Konfiguration zwischen v2 und v3 unterscheidet, sind hervorgehoben.
...
Für die MLP-Baseline waren keine architekturspezifischen Tuning-Läufe erforderlich. Ihre Konfiguration wurde unverändert in den beiden finalen Vergleichsläufen final_v2 und final_v3 eingesetzt. final_v2 wurde bei rund 16,6 Millionen Schritten beendet, während final_v3 das vollständige Budget von 30 Millionen Schritten durchlief. Die Konfiguration wurde einmalig aus den dokumentierten PPO-Standardwerten des Frameworks abgeleitet, im Rahmen des Vergleichsdesigns eingefroren und während des Laufs zu keinem Zeitpunkt verändert. Dieser geringe Aufwand ist unmittelbar durch die Framework-Integration erklärbar: Das MLP ist als Standardarchitektur nativ in Unity ML-Agents enthalten und damit die am einfachsten zu implementierende der drei Architekturen. Weder die Netzarchitektur noch der Python-Trainer mussten angepasst werden, der Integrationsaufwand beschränkte sich auf die architekturunabhängige Anbindung von Umgebung und Agent, die zugleich die gemeinsame Grundlage aller drei Architekturen bildet (@sec:mlp-integration).

Abweichungen im Sinne dieses Abschnitts traten bei der MLP-Baseline dementsprechend weder auf der Ebene der Hyperparameter noch im Trainingsablauf auf. Die MLP-Konfiguration war in beiden Läufen (v2 und v3) identisch. Sie liegen vielmehr auf der Verhaltensebene, in den strukturellen Grenzen einer gedächtnislosen Policy, wie sie in @sec:mlp-erwartungen formuliert wurden. Ob und in welchem Ausmaß diese erwarteten Grenzen im Trainingsverlauf tatsächlich beobachtbar sind, wird zusammen mit den finalen Ergebniszahlen in @sec:eval-mlp-baseline dargestellt und eingeordnet.
...
Das Ergebnis dieser Iterationen ist die best-getunte Transformer-Konfiguration, die im finalen Lauf v3 zum Einsatz kommt. Sie umfasst die vier Werte, die den v3-Transformer von der v2-Fassung unterscheiden. Das sind ein abgesenktes beta, ein konstantes beta_schedule, eine konstante learning_rate_schedule und eine reduzierte curiosity.strength (@tab:yaml-abweichungen). Der frühere finale Lauf v2 fuhr den Transformer dagegen noch mit einer nicht-optimalen Konfiguration, die vor Abschluss dieser Iterationen entstand. Warum trotzdem beide Läufe berichtet werden und wie sich das auf die Interpretation auswirkt, behandelt Abschnitt 5.5.



== Evaluationsprotokoll <sec:evalprotokoll>
Der zentrale Erfolgsmaßstab ist die Erfolgsrate, verstanden als Anteil der Episoden, in denen der Agent das Ziel erreicht. Als Fehlschlag gilt jede Episode, in der der Agent in Lava oder ein Loch fällt oder in der die Zeitgrenze überschritten wird. Ermittelt wird dieser Wert nicht aus dem laufenden Training, sondern aus separaten Evaluationsläufen, die nach Abschluss des Trainings im Inferenzmodus auf einem zurückgehaltenen Kartensatz durchgeführt werden. Dieselbe Auswertung wird auf beide finalen Checkpoint-Sätze angewandt, also auf den Lauf v2 und den Lauf v3, und verwendet für beide dieselbe Testharness, dieselbe Szene, dieselben 155 Karten und dieselben paarweisen Seeds. Für v3 werden die Checkpoints zuvor von CUDA auf CPU konvertiert. Da die Transformer-Architektur nicht unter Barracuda lauffähig ist, erfolgt diese Inferenz in Python.

Auf derselben Erfolgsgröße setzt die Konvergenzgeschwindigkeit auf. Die Konvergenzgeschwindigkeit erfasst, wie rasch ein Modell verlässlich erfolgreich agiert, und misst dazu die kumulierten Behavior-Steps bis zu dem Punkt, an dem die rollierende Erfolgsrate im gleitenden Fenster nachhaltig 80 Prozent überschreitet. Bleibt diese Schwelle unerreicht, wird der Wert rechtszensiert. Der finale Leistungsscore beschreibt hingegen das erreichte Leistungsniveau und entspricht dem mittleren kumulativen Episoden-Reward. Dieser Reward stammt aus der trainingsseitigen Aufzeichnung und nicht aus den Evaluationsläufen, weil deren Ausgabedaten keinen Reward enthalten.
...
=== Primärmetriken
Die maßgebliche Erfolgsrate stammt aus der separaten Held-out-Evaluation im Inferenzmodus, während ihr zeitlicher Verlauf über die Größen SuccessRate und RollingSuccessRate aus TensorBoard abgelesen wird. Aus derselben rollierenden Trajektorie ergibt sich die Konvergenzgeschwindigkeit. Die mittlere Episodenlänge stützt sich auf die trainingsseitige Größe EpisodeLength und wird für erfolgreiche Episoden um StepsToGoal ergänzt. Dass sich diese Länge nur trainingsseitig bestimmen lässt, liegt an den Ausgabedaten der Evaluationsläufe, die allein die benötigte Zeit in Sekunden, aber keinen Schrittzähler führen. Aus demselben Grund bleibt der finale Leistungsscore eine trainingsseitige Größe und wird über den kumulativen Reward in TensorBoard bestimmt.

Alle Primärmetriken werden je Lauf ausgewiesen. Besonders der finale Leistungsscore ist zu beachten, denn der mittlere kumulative Reward fällt zwischen v2 und v3 deutlich unterschiedlich aus und wird deshalb nicht als lauf-übergreifende Einzelzahl dargestellt.

Eine Kollisionsrate lässt sich nicht ausweisen, weil dafür keine Instrumentierung vorliegt und weder ein Kollisionsereignis noch ein Zähler erfasst wird. Sie entfällt deshalb als Metrik. Zur inhaltlichen Deutung des Agentenverhaltens steht demgegenüber eine Reihe aus TensorBoard ableitbarer Diagnosegrößen bereit. Zu ihnen zählen die Todesursachen nach Lava, Loch und Timeout, die verbleibende Distanz zum Ziel und der Distanzfortschritt. Das Verhalten an riskantem Terrain wird über Lava-Überquerungen und -Versuche, Loch-Überflüge, Wandkletterkontakte und die Zahl der Sprünge je Episode erfasst und um den Trainingsschritt der ersten Lava-Überquerung ergänzt. Die Belohnungsstruktur bilden die aufsummierten Reward-Anteile aus Sichtlinie und potentialbasiertem Reward-Shaping ab, und der Fortschritt im Curriculum zeigt sich in der jeweiligen Curriculum-Phase samt den Erfolgsraten je Phase und Gate.
...
=== Konvergenzmetrik (F2)
// Konvergenzgeschwindigkeit := kumulative Behavior-Steps bis zum NACHHALTIGEN
//   Überschreiten von 80% Erfolgsrate. Definition:
// - Signal: TB-RollingSuccessRate, eigenes Auswertungsfenster W = 200 Episoden  // FIXIEREN W
// - Referenzstufe: Hard (Phase 7)                                               // FIXIEREN (Hard vs. Loop-Aggregat)
// - "nachhaltig": >=80% erreicht UND >=80% über S=3 Folgefenster gehalten        // FIXIEREN S
// - Schritt-Einheit: kumulative Behavior-Steps (TB-x-Achse/Checkpoint-Step) [§13.1]
// - Zensur: Bedingung nie erfüllt -> rechtszensiert bei laufspezifischem max_steps
//   (v2 16,6 M, v3 30 M) ("nicht erreicht"), nicht imputieren
// - kein Code-Gegenstück im System [§12-#3] -> rein auswertungsseitig

Die Konvergenzgeschwindigkeit gibt an, nach wie vielen kumulierten Behavior-Steps ein Training die Erfolgsrate von 80 Prozent nachhaltig überschreitet. Gemessen wird sie in eben diesen kumulierten Behavior-Steps, also entlang der Fortschrittsachse der aufgezeichneten Verläufe beziehungsweise des jeweiligen Checkpoint-Steps.

Grundlage der Auswertung ist die rollierende Erfolgsrate (TB-RollingSuccessRate), die über ein eigenes Auswertungsfenster von 200 Episoden bestimmt wird. Dieses Fenster von 200 Episoden entspricht dem trainingsseitig verwendeten Erfolgsfenster. Als Referenzstufe dient die Stufe Hard aus Phase 7, auf die sich die betrachtete Erfolgsrate bezieht.

Ein einmaliges Erreichen der Schwelle genügt nicht. Als nachhaltig gilt die Überschreitung erst, wenn die Erfolgsrate 80 Prozent erreicht und diese Marke über drei folgende Fenster hält. Diese Bedingung stellt sicher, dass die Konvergenz auf einem stabilen Leistungsniveau beruht und nicht auf einer einzelnen Schwankung.

Erreicht ein Lauf die Bedingung bis zum Ende nicht, wird er bei dem für ihn gültigen max_steps rechtszensiert und als nicht erreicht gekennzeichnet, für v2 bei 16,6 Millionen und für v3 bei 30 Millionen Behavior-Steps. Diese laufspezifische Grenze ist nötig, damit die Kennzeichnung als nicht erreicht zwischen beiden Läufen vergleichbar bleibt. Ein Wert wird in diesem Fall nicht imputiert, da eine solche Ersetzung ein tatsächlich nicht beobachtetes Ergebnis vortäuschen würde. Die Metrik besitzt kein Gegenstück im Code des Systems und wird ausschließlich auswertungsseitig gebildet.
...
Einschränkend bleibt festzuhalten, dass alle Modelle auf einem einzigen Trainings-Seed 42 beruhen, der für v2 und v3 identisch ist. Die statistischen Aussagen gelten deshalb für die konkret trainierten Modellinstanzen und beschreiben deren Generalisierung auf der Ebene neuer Karten. Eine Aussage über die jeweilige Architektur im Erwartungswert über verschiedene Seeds lassen sie nicht zu, da hierfür ein systematischer Seed-Sweep nötig wäre. Die beobachtete Divergenz zwischen v2 und v3 geht folglich nicht auf verschiedene Seeds zurück, denn beide verwenden Seed 42, sondern auf Unterschiede in Budget, virtueller Umgebung und Stochastik sowie beim Transformer auf eine Konfigurationsänderung. Sie ist als Signal für die Reproduzierbarkeit zu verstehen und ersetzt keinen Seed-Sweep.

Methodisch ist zudem sicherzustellen, dass beide Läufe vollständig und für alle drei Architekturen berichtet werden. Ein Vergleich darf nicht aus dem LSTM des Laufs v2 und dem Transformer des Laufs v3 zusammengesetzt werden, denn eine solche Auswahl wäre Rosinenpickerei und würde die Umkehr der Rangfolge verdecken, die gerade den interessanten Befund darstellt.
...
Damit der spätere Architekturvergleich wissenschaftlich belastbar ist, wurde vor dem Training ein Grundprinzip festgelegt: Zwischen den zu vergleichenden Agentenvarianten soll sich möglichst nur eine Variable unterscheiden die Modellarchitektur beziehungsweise der Gedächtnistyp, während alle übrigen Randbedingungen identisch bleiben. „Hierzu wurden Belohnungsstruktur, Umgebungskonfiguration und die gemeinsamen Trainingsparameter dokumentiert und für alle Läufe eingefroren. Vollständig erreicht wird dieses Ideal jedoch nur für MLP und LSTM. Der Transformer-Agent weicht in mehreren Trainingshyperparametern von der Referenzkonfiguration ab. Diese begründeten, architekturspezifischen Abweichungen sind in @sec:yaml-abweichungen aufgeführt." Die in den folgenden Teilabschnitten beschriebene Konfiguration der MLP-Baseline definiert damit zugleich die Referenzkonfiguration für alle weiteren Modellvarianten dieser Arbeit. Der Architekturvergleich stützt sich auf zwei finale Vergleichsläufe, im Folgenden als v2 und v3 bezeichnet. Beide verwenden dieselbe eingefrorene Referenzkonfiguration; in der Trainer-Konfiguration unterscheiden sie sich ausschließlich in vier Hyperparametern des Transformers (@sec:yaml-abweichungen), die MLP- und LSTM-Konfiguration ist zwischen beiden Läufen identisch. Darüber hinaus unterscheiden sich die Läufe in laufweiten Randbedingungen (Trainingsbudget, Parallelität, Trainingsumgebung), die in Abschnitt 5.3 abgegrenzt werden. Lauf v3 enthält die überarbeitete, als Referenz vorgesehene Transformer-Konfiguration, während in v2 für den Transformer noch Werte verwendet wurden, die sich im Verlauf der Arbeit als ungünstig erwiesen. Daraus folgt für die Bewertung: Der Transformer wird in v2 nicht unter bestmöglicher Konfiguration verglichen, weshalb für ihn das v3-Ergebnis maßgeblich ist. Da MLP und LSTM in beiden Läufen identisch konfiguriert sind, lassen sich Unterschiede in ihren Ergebnissen zwischen v2 und v3 nicht auf die Architektur zurückführen, sondern auf das erhöhte Trainingsbudget (16,6 M gegenüber 30 M Schritten). Die ergebnisbezogene Diskussion dieses Vorbehalts erfolgt in @sec:evaluation.
...
=== Problemklasse D: Konfigurations-Drift Unity -
Python 
...
ML-Agents bringt zwar eine eigene LSTM-Unterstützung mit, aber  das Gedächtnismodul wurde dennoch selbst implementiert (LSTMMemory, siehe Abschnitt 6.2.2). Es wird über denselben venv-Patch eingebaut wie die Transformer-Policy. Der Grund dafür ist die Vergleichbarkeit, da das Modul als exaktes Spiegelbild des TransformerMemory gebaut ist. Es hat die gleichen Ein- und Ausgabeformen, verarbeitet die Trainingssequenzen auf die gleiche Weise und sitzt an derselben Stelle im Netz.
 Beide Gedächtnisvarianten unterscheiden sich also nur in ihrer inneren Logik, nicht in der Verdrahtung. Interessant ist der Größenunterschied. Das LSTM-Modul hat rund 198.000 Parameter, das Transformer-Modul ca. 1,07 Millionen. Der Vergleich stellt bewusst zwei Gedächtnis-Mechanismen gegenüber und keine gleich großen Netze.
...
Der grundlegende Aufbau des Trainingssystems in vorherigen Kapiteln beschrieben und gilt für alle drei Architekturen gleichermaßen. Für die Integration der MLP-Baseline ist daran nur relevant, dass Unity-Umgebung und Python-Trainingsprozess (mlagents-learn) während des Trainings gleichzeitig laufen und der Agent zu jedem Entscheidungszeitpunkt Beobachtungen an den Trainer sendet und dessen Aktionen zurückerhält. Die folgenden Abschnitte konzentrieren sich daher auf die architekturunabhängige Anbindung von Umgebung und Agent, die den eigentlichen Integrationsaufwand der Baseline ausmacht.
...
Als Funktionsnachweis der Trainings- und Evaluationspipeline wird zunächst die MLP-Baseline betrachtet (siehe @sec:eval-mlp-baseline). Im finalen Vergleichslauf (final_v3, 30 Mio. Trainingsschritte) erreicht der Agent auf der Hard-Stufe des Curriculums eine maximale Erfolgsrate von 88,6 % bei einem mittleren kumulativen Reward von 16,4. Die häufigsten Fehlschläge entstehen durch Lava-Kollisionen (15,4 %) sowie Timeouts (12,3 %).
...
Ein differenzierteres Bild ergibt der Vergleich der Todesursachen. Während bei der MLP-Baseline Lava-Kollisionen (15,4 %) und Timeouts (12,3%) in ähnlicher Größenordnung auftreten, dominieren bei LSTM und Transformer die Lava-Tode (24,7 % beziehungsweise 18,7 %), während Timeouts nur noch einen geringen Anteil ausmachen (2,4 % beziehungsweise 3,3 %).
...
Bei der Einordnung der Konvergenzgeschwindigkeit ist zu beachten, dass Abbildung @fig:traingsverlauf_v3_final die jeweils beste erreichte Erfolgsrate darstellt, nicht jedoch den vollständigen Verlauf der rollierenden Erfolgsrate, auf dem die in @sec:evalprotokoll definierte Konvergenzmetrik basiert. Die Konvergenzbedingung verlangt, dass die Erfolgsrate die 80-%-Schwelle nachhaltig über drei aufeinanderfolgende Auswertungsfenster überschreitet. Diese Eigenschaft lässt sich aus den verfügbaren Daten nicht belastbar rekonstruieren.
Für LSTM und Transformer ist die Aussage dennoch eindeutig: Beide Architekturen bleiben bereits mit ihren jeweiligen Bestwerten (74,3 % beziehungsweise 79,0 %) unterhalb der 80-%-Schwelle und gelten damit sicher als rechtszensiert. Die MLP-Baseline erreicht dagegen einen Bestwert von 88,6 % und kommt der definierten Konvergenzbedingung am nächsten. Ob die Schwelle auch nachhaltig gehalten wurde, lässt sich auf Basis der vorliegenden Daten jedoch nicht sicher feststellen.
Für den direkten Vergleich zwischen LSTM und Transformer liefern die Trainingsdaten dennoch interessante Hinweise. Der Transformer erreicht mit 79,0 % die höhere maximale Erfolgsrate gegenüber dem LSTM (74,3 %), benötigt dafür jedoch deutlich mehr Trainingsschritte. Während das LSTM sein Leistungsniveau bereits früh im Training erreicht, erfolgt der wesentliche Leistungsanstieg des Transformers erst nach etwa 21 bis 22 Mio. Trainingsschritten.
Die Trainingsverläufe bestätigen H2 damit in beiden Teilen. Der Transformer erzielt die höhere beobachtete Bestleistung, benötigt dafür jedoch deutlich mehr Trainingsschritte als das LSTM. Die einzige verbleibende Unsicherheit betrifft nicht die Richtung dieses Befunds, sondern die exakte Quantifizierung der Konvergenzgeschwindigkeit. Diese kann anhand der verfügbaren Daten nicht mit der in @sec:evalprotokoll definierten Präzision bestimmt werden
...
Wie @fig:final_v3 zeigt, kollabiert die Erfolgsrate des LSTM auf allen Schwierigkeitsstufen nahezu vollständig (Easy 8 %, Medium 4 %, Hard 1 %, Giant 0 %) – trotz eines Trainings-Bestwerts von 74,3 % in Phase 7 (Abschnitt 8.2). MLP und Transformer generalisieren dagegen deutlich robuster und erreichen durchweg ähnliche Erfolgsraten (Easy 78 % beziehungsweise 84 %, Medium 84 % beziehungsweise 80 %, Hard 64 % beziehungsweise 56 %, Giant 40 % beziehungsweise 52 %).

Der Overfitting-Index (Trainings-Erfolgsrate im Endstand der jeweils passenden Phase minus Held-out-Erfolgsrate, @sec:evalprotokoll) lässt sich aus den verfügbaren Daten nicht exakt bestimmen, da hierfür die Trainings-Erfolgsraten am Ende der jeweils korrespondierenden Curriculum-Phasen benötigt würden. Eine qualitative Einordnung ist dennoch möglich. Beim LSTM ist der Index aufgrund der extrem niedrigen Held-out-Erfolgsraten offensichtlich sehr groß und deutet auf eine starke Überanpassung an die Trainingskarten hin. Bei MLP und Transformer liegen Held-out-Erfolgsrate und die rollierende Trainings-Erfolgsrate am Phasenende (nicht der in @sec:architekturvergleiche berichtete Bestwert, sondern deren dortiges Endniveau von rund 57–62 %) dagegen in vergleichbarer Größenordnung, sodass kein ausgeprägtes Overfitting erkennbar ist.
...
// ============================================================================
// DATENBASIS: final_v3 (30 M Steps, volles Budget) als Hauptbasis.
// final_v2 (16,6 M Steps) dient als Kontrast zur Budget-Abhängigkeit.
// KORREKTUR ggü. SSOT §13.1/§9.2: v2 und v3 nutzten denselben Transformer
// (interne Bezeichnung V22, identische Konfiguration) — SSOT-Stelle anpassen!
...
Die erste Teilfrage F1 richtet sich auf den Mehrwert temporalen Gedächtnisses gegenüber der gedächtnislosen MLP-Baseline. Der Held-out-Test des Hauptlaufs ergibt hier kein einheitliches Bild. Der Transformer übertrifft das MLP auf den einfachen Karten und auf der Out-of-Distribution-Stufe Giant, bleibt auf den mittleren und schweren Karten jedoch hinter ihm zurück. Das LSTM fällt über alle Kategorien deutlich unter die Baseline. Ein durchgängiger Mehrwert temporalen Gedächtnisses lässt sich daraus nicht ableiten: Hypothese H1 wird in ihrer allgemeinen Form nicht bestätigt, allenfalls zeigt der Transformer an den Rändern des Schwierigkeitsspektrums einen selektiven Vorteil. Dieser Befund gilt zudem nur für den Hauptlauf; im Budget-Kontrast final_v2 war das LSTM die stärkste Architektur, sodass die Antwort auf F1 budgetabhängig ausfällt — wobei dieser Budget-Kontrast nur quasi-kontrolliert und für den Transformer zusätzlich mit geänderten Hyperparametern konfundiert ist (@sec:bewertungsmatrix).

Die zweite Teilfrage F2 vergleicht LSTM und Transformer hinsichtlich Trainingsdynamik und Leistungsfähigkeit. Auf der Trainingsseite liefert die Konvergenzmetrik aus @sec:evalprotokoll keinen messbaren Unterschied: Keine der beiden Gedächtnisarchitekturen überschreitet die 80-Prozent-Schwelle der rollierenden Erfolgsrate nachhaltig, beide Läufe sind im Sinne der Metrikdefinition rechtszensiert. Auch die finalen Leistungsscores liegen praktisch gleichauf, und die Lernkurven verlaufen über weite Strecken nahezu deckungsgleich, mit einem geringfügigen Vorsprung des Transformers am Laufende. Trainingsseitig wird H2 damit nicht bestätigt, da der Transformer weder eine klar höhere Endleistung noch eine messbar langsamere Konvergenz zeigt. Der praktisch entscheidende Unterschied der beiden Architekturen tritt erst auf den zurückgehaltenen Karten zutage, wo sie bei vollem Budget eine Größenordnung trennt (@tab_generalisierung_ergebnisse). In dieser Lesart stützt das Ergebnis den auf die Endleistung bezogenen Teil von H2 qualitativ, während der auf Konvergenz bezogene Teil mangels erreichter Schwelle unbeantwortet bleibt.

Die dritte Teilfrage F3 betrifft die Generalisierung auf unbekannte Kartenlayouts. Für den Hauptlauf ist die Antwort eindeutig: Am zuverlässigsten generalisieren MLP und Transformer, während das LSTM ein ausgeprägtes Overfitting-Muster zeigt, erkennbar an der großen Lücke zwischen hoher Trainings- und sehr niedriger Held-out-Erfolgsrate. Damit wird Hypothese H3 widerlegt, denn nicht die Gedächtnisarchitekturen, sondern die gedächtnislose Baseline weist den geringsten Abstand zwischen Trainings- und Generalisierungsleistung auf. Auch diese Antwort gilt nur je Budgetstufe: Im Lauf final_v2 kehrt sich die Rangfolge um. Die Deutung dieses Wechsels als Budget-Effekt beziehungsweise als LSTM-Overfitting sowie ihre Grenzen behandeln @sec:bewertungsmatrix und @sec:limitationen-uebertragbarkeit.
