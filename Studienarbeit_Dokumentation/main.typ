...
Zur Laufzeit instanziiert der MapGenerator nur ein bestimmtes Layout als 3D-Szene. Er fungiert allein als Renderer. Wie in der Offline-Pipeline (@sec:prozgen) detailliert beschrieben, erfolgen die Hinderniskonstruktion und die Überprüfung der Lösbarkeit außerhalb des MapGenerators. Erst diese Trennung von Erzeugung und Darstellung erlaubt es, Karten vor Trainingsbeginn zu erzeugen, einzufrieren und als ungesehenes Evaluations-Set zurückzuhalten. Damit ist die in @sec:messbarkeit begründete Voraussetzung für den Nachweis von Generalisierung (F3) erfüllt, deren konkrete Messung @sec:evalprotokoll beschreibt.
